...
Class Clientes
{
	...
	public function insertar($id,$n1,$n2,$a1,$a2,$correo,$pass)
	{
		...
		$verificacion="SELECT * from usuarios where identificacion='$id'";
		...
		{	
			############################Envio de info################################
			$enviar1= "INSERT INTO usuarios(identificacion,p_nombre,s_nombre,p_apellido,s_apellido,correo,contrasena,estado) VALUES ('$id','$n1','$n2','$a1','$a2','$correo',aes_encrypt('$credencial','$permiso'),'$visible')";
			$insertar1=$this->db->query($enviar1);
			header("Location: clientes_con_ctrl");
		}	
	}
	...
	public function consulta()
	{
		$estado=1;
		$info=array();
		$rol="SELECT * from usuarios where estado='$estado'";
	...
	{
		$indice=trim($dato);
		$sql="SELECT * from usuarios where identificacion='$indice'";
	...
	{
		$act="UPDATE usuarios set identificacion='$id',p_nombre='$n1',s_nombre='$n2',p_apellido='$a1',s_apellido='$a2',correo='$correo' where identificacion='$dato'";
	...
	{
		$estado=0;
		$elim="UPDATE usuarios set estado='$estado' where identificacion='$indice'";
	...
	{
		$sql="SELECT * from usuarios where identificacion='$_SESSION[id]'";
	...
	public function permisos($id,$rol_u)
	{
		...
		if ($id==$dato)
		{
			$rol="SELECT * from roles where t_permiso='$rol_u'";
			$envio= $this->db->query($rol);
			$trae=mysqli_fetch_array($envio);
			$asigna=$trae['u_rol'];
			$enviar="INSERT INTO permisos (id_usuario,id_rol) VALUES('$id','$asigna')";
