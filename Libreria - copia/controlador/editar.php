...
if (isset($_GET['editar']))
	...
	$clave="julian123";
	$intento=aes_desencriptar($identificacion,$clave);
	$intento_e=$identificacion;
	echo "<script>alert('$intento_e')</script>";
	function aes_desencriptar($intento)
	{
		$td=mcrypt_module_open(MCRYPT_RIJNDAEL_128,'',MCRYPT_MODE_CBC,'');
		$iv_size_hex=mcrypt_enc_get_iv_size($td)*2;
 		$iv=pack('H*',substr($intento_e,0,$iv_size_hex));
