	...
	public function arreglo($consulta)
	{
		...
		while ($informe=$consulta->fetch_array(MYSQLI_ASSOC))
		{
			$objPHPExcel->setActiveSheetIndex(0)
						...
						->setCellValue("B$i",$informe['p_nombre'])
						->setCellValue("C$i",$informe['s_nombre'])
						->setCellValue("D$i",$informe['p_apellido'])
						->setCellValue("E$i",$informe['s_apellido'])
						->setCellValue("F$i",$informe['correo'])
						->setCellValue("G$i",$informe['estado']);
