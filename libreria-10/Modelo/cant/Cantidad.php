<?php 
/*Procesamiento de los datos de las cantidades de los libros que ingresaron por pedido o por devoluciones esta tien ecomo no vedad el  uso de la fn now () y sum() de mysql
Glosario
now()=devuelve fecha actual como minutos y seg 
sum()= suma de valores 
*/
Class Cantidad
{
	...
	{
		$verificacion="SELECT * from libro where isbn='$isbn'";
	...
	{
		$consulta= "SELECT isbn,isbn_c,libro,sum(cant_ingresada) as entrantes,sum(cant_reservada) as prestados,sum(cant_lib_dev) as devolucion from historial inner join libro WHERE isbn='$dato' and isbn_c=isbn";
