...
Class Libros
{
	...
	public function insertar($id,$nom,$fec,$ed,$cat,$aut,$p_aut)
	{
		$fec=str_replace(" ","",$fec);
		//verificacion
		$verifica="SELECT * FROM libro where isbn='$id'";
		...
		else
		{
			#
			//setencias sql
			$enviar= "INSERT INTO `libro`(`isbn`, `libro`, `fecha`, `editorial`, `categoria`, `autor`, `pais_de_autor`,compra,estado)  VALUES ('$id','$nom','$fec','$ed','$cat','$aut','$p_aut','$this->estado','$this->activo')";
				$van=$this->db->query($enviar);//insercion de los datos
				header("Location: libros_con_ctrl.php");
		}
	}
	...
	{
		$informacion=array();
		$sql="SELECT * FROM libro where estado='$this->activo'";
	...
	{
		$sql="SELECT * FROM libro where isbn='$dato'";
		$envio=$this->db->query($sql);
	...
	public function actualizar($id,$nom,$fec,$ed,$cat,$aut,$p_aut,$est,$indice)
	{
		...
		$actualizar= "UPDATE libro SET isbn='$id', libro='$nom', fecha='$fec', editorial='$ed', categoria='$cat', autor='$aut', pais_de_autor='$p_aut', compra='$est', estado='$disponibilidad' WHERE isbn='$indice'";
	...
	{
		$elim="UPDATE libro set estado='$this->inactivo' where isbn='$indicio'";
